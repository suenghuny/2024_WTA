...
def modeler(data, visualize, size, detection_by_height, tick, simtime_per_framerate, ciws_threshold, action_history_step,
            cum_lose_ratio,
            epsilon = 20, discr_n = 10,
...
class Environment:
    def __init__(self,
        ...
        self.now = 0
        self.cum_lose_ratio= cum_lose_ratio
        self.visualize = visualize
        self.temp_termination = False
        self.tick = tick
        self.action_history_step = action_history_step
        self.air_alert_distance_blue = air_alert_distance_blue
        ...
        self.fri_sam_list = list()
        self.foe_sam_list = list()

        self.fri_ssm_list = list()
        self.foe_ssm_list = list()

        self.fri_ship_list = list()
        self.foe_ship_list = list()
                ...
                if len(ship.air_engagement_managing_list) < ship.air_engagement_limit:
                    ...
                    if speed_normalizing == True:
                        ...
                        distance_range_1 = [cal_distance(ship, ssm) for ssm in ship.ssm_detections if
                                   cal_distance(ship, ssm) > ship.l_sam_max_range]
                        # 1구간 : > l_sam range
                        ...
                        #print(distance_range_3, ship.m_sam_max_range)

                        distance_range_4 = [cal_distance(ship, ssm) for ssm in ship.ssm_detections
                                   if cal_distance(ship, ssm) <= ship.ciws_max_range]

                    if ship.side == 'blue':
                        air_alert_distance = self.air_alert_distance_blue
                    else:
                        air_alert_distance = self.air_alert_distance_yellow
                    for ssm in ship.ssm_detections:
                        if (cal_distance(ship, ssm) <= air_alert_distance):
                            air_alert = True
                            break
                        else:
                            air_alert = False
  ...
  #      reward = 0
        for i in range(len(self.friendlies_fixed_list)):
            ...
            if ship.status != 'destroyed':
                ...
                if self.visualize == True:
                    ship.show()
                    self.fri_ship_list.append([self.now, ship.id, ship.position_x, ship.position_y])
        ...
        for i in range(len(self.enemies_fixed_list)):
            ...
            if ship.status != 'destroyed':
                ...
                if self.visualize == True:
                    ship.show()
                    self.foe_ship_list.append([self.now, ship.id, ship.position_x, ship.position_y])
                    font1 = pygame.font.Font(None, 15)
                    img1 = font1.render('SSM : {}_LSAM : {}_MSAM : {}'.format(len(ship.ssm_launcher), len(ship.l_sam_launcher), len(ship.m_sam_launcher)), True, (150, 120, 15))
                    #self.screen.blit(img1, (ship.position_x, ship.position_y+20))
        ...
        for ssm in temp_flying_ssms_friendly:
            ...
            if self.visualize == True:
                ssm.show()
                font1 = pygame.font.Font(None, 15)
                self.fri_ssm_list.append([self.now, ssm.id, ssm.position_x, ssm.position_y])
        ...
        for ssm in temp_flying_ssms_enemy:
            ...
            if self.visualize == True:
                ssm.show()
                self.foe_ssm_list.append([self.now, ssm.id, ssm.position_x, ssm.position_y])
                font1 = pygame.font.Font(None, 15)
                img1 = font1.render('{} ,{}'.format(ssm.id, np.round(cal_distance(ssm, ssm.target))), True, (150, 120, 15))
                #self.screen.blit(img1, (ssm.position_x, ssm.position_y))
                img1 = font1.render('est, {}, {}'.format(ssm.id, ssm.fly_mode), True, (0,250,0))
        ...
        for sam in temp_flying_sams_friendly:
            ...
            if self.visualize == True:
                sam.show()
                self.fri_sam_list.append([self.now, sam.id, sam.position_x, sam.position_y])
                font1 = pygame.font.Font(None, 15)
        ...
        for sam in temp_flying_sams_enemy:
            ...
            if self.visualize == True:
                sam.show()
                self.foe_sam_list.append([self.now, sam.id, sam.position_x, sam.position_y])
                font1 = pygame.font.Font(None, 15)
        ...
        if self.visualize == True:
            ...
            img1 = font1.render('Cumulative Non-lose Rate {}'.format(self.cum_lose_ratio), True, (0,250,0))
            self.screen.blit(img1, (300, 20))

            img2 = font1.render('Mean Reward  {}'.format(np.round(self.mean_reward)), True, (0, 250, 0))
            self.screen.blit(img2, (300, 40))

            img2 = font1.render('Episode Reward  {}'.format(self.cum_reward), True, (0, 250, 0))
            self.screen.blit(img2, (300, 60))
        ...
        if rl == True:
            if done == True:
                df_fri_sam_list = pd.DataFrame(self.fri_sam_list)
                df_fri_sam_list.columns = ['fri_sam_time', 'fri_sam_id', 'fri_sam_x', 'fri_sam_y']
                df_fri_sam_list = df_fri_sam_list.reset_index(drop=True)

                df_foe_sam_list = pd.DataFrame(self.foe_sam_list)
                df_foe_sam_list.columns = ['foe_sam_time', 'foe_sam_id', 'foe_sam_x', 'foe_sam_y']
                df_foe_sam_list = df_foe_sam_list.reset_index(drop=True)

                df_fri_ssm_list = pd.DataFrame(self.fri_ssm_list)
                df_fri_ssm_list.columns = ['fri_ssm_time', 'fri_ssm_id', 'fri_ssm_x', 'fri_ssm_y']
                df_fri_ssm_list = df_fri_ssm_list.reset_index(drop=True)
                print(df_fri_ssm_list)

                df_foe_ssm_list = pd.DataFrame(self.foe_ssm_list)
                df_foe_ssm_list.columns = ['foe_ssm_time', 'foe_ssm_id', 'foe_ssm_x', 'foe_ssm_y']
                df_foe_ssm_list = df_foe_ssm_list.reset_index(drop=True)

                df_fri_ship_list = pd.DataFrame(self.fri_ship_list)
                df_fri_ship_list.columns = ['fri_ship_time', 'fri_ship_id', 'fri_ship_x', 'fri_ship_y']
                df_fri_ship_list = df_fri_ship_list.reset_index(drop=True)

                df_foe_ship_list = pd.DataFrame(self.foe_ship_list)
                df_foe_ship_list.columns = ['foe_ship_time', 'foe_ship_id', 'foe_ship_x', 'foe_ship_y']
                df_foe_ship_list = df_foe_ship_list.reset_index(drop=True)

                df_fri_sam_list.to_csv('fri_sam.csv', index=False)
                df_foe_sam_list.to_csv('foe_sam.csv', index=False)
                df_fri_ssm_list.to_csv('fri_ssm.csv', index=False)
                df_foe_ssm_list.to_csv('foe_ssm.csv', index=False)
                df_fri_ship_list.to_csv('fri_ship.csv', index=False)
                df_foe_ship_list.to_csv('foe_ship.csv', index=False)
