...
class Environment:
    ...
    def get_feature(self, ship, target, action_feature = False):
        #noise = ((target.position_x - ship.position_x) ** 2 + (target.position_y - ship.position_y) ** 2) ** 0.5
        #noise = norm.rvs()
        #print(target.v_x, target.v_y, target.speed)
        target_position_x = target.position_x
        target_position_y = target.position_y
        target_v_x = target.v_x
        target_v_y = target.v_y
        #print(target_position_x)
        if action_feature == False:
            r = ((target_position_x - ship.position_x) ** 2 + (target_position_y - ship.position_y) ** 2) ** 0.5 / ((target.speed+ship.speed)*2000)
            v = ((target_v_x - ship.v_x) ** 2 + (target_v_y - ship.v_y) ** 2) ** 0.5 / (target.speed+ship.speed)
            theta_r = math.atan2(target_position_y - ship.position_y, target_position_x - ship.position_x)
            theta_v = math.atan2(ship.v_y - target_v_y, ship.v_x - target_v_x)
            ...
            r = ((target_position_x - ship.position_x) ** 2 + (target_position_y - ship.position_y) ** 2) ** 0.5 / (
