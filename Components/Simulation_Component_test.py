...
class Missile:
    def __init__(self, env, launcher, spec):
        ...
        if self.cla == 'MSAM':
            if self.env.visualize == True:
                if self.launcher.side == 'blue':
                    ...
                    self.put_img('Image\m_sam_yellow.png')

                self.change_size(7, 7)
        if self.cla == 'LSAM':
            if self.env.visualize == True:
                if self.launcher.side == 'blue':
                    self.put_img('Image\l_sam.png')
                else:
                    self.put_img('Image\l_sam_yellow.png')
                self.change_size(7, 7)
        if self.cla == 'SSM':
            if self.env.visualize == True:
                if self.launcher.side == 'blue':
                    self.put_img('Image\ssm.png')
                else:
                    self.put_img('Image\ssm_yellow.png')
                self.change_size(9, 9)
        ...
        self.last_action_feature = dict()
        for key, value in self.env.data.ship_data.items():
            if value['side'] == 'blue':
                k = key-1
                self.last_action_feature[k] = '-'



    def destroying(self, enemies, ships, flying_ssms_friendly, flying_ssms_enemy, flying_sams_friendly,
                   flying_sams_enemy):

        # if self.launcher.side =='yellow':
        #     if self.cla == 'SSM':
        #         if self.target.cla == 'decoy':
        #             print(self.id, self.original_target.cla, self.target.cla)
    ...
    def get_object_angle_noise(self):
        ...
        return math.atan2(-(self.estimated_hitting_point_y - self.position_y),
                          self.estimated_hitting_point_x - self.position_x)
...
class Ship:
    ...
    def target_allot_by_action_feature(self, action_feature):
        ...
        if (target_id >= 1) and (target_id <= len_limit):  # 대함표적에 대한 prelaunching process logic
            ...
            idle_ssm_launcher = [missile for missile in self.ssm_launcher if missile.status == 'idle']  # idle한 유도탄 중에서 유도탄을 할당
