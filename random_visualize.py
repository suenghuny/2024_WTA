    ...
    while not done:
        if env.now % (decision_timestep) <= 0.00001:
            ...
            for i in range(len(env.friendlies_fixed_list)):
                ...
                action_blue, prob, mask, a_index, graph_embedding, graph_feature, output = agent.sample_action_visualize(ship_feature, missile_node_feature,heterogeneous_edges, avail_action_blue[i],action_feature, random = True)
...
if __name__ == "__main__":
    ...
    records = list()
    datasets = [i for i in range(28, 29)]
    non_lose_ratio_list = []
    raw_data = list()
    cum_lose_ratio = 0
    cum_reward = list()
    ...
    for inception_angle in [90]:
        ...
        for dataset in datasets:
            ...
            for e in range(20):
                ...
                cum_lose_ratio = np.mean(non_lose_records)
                mean_reward = np.mean(cum_reward)
